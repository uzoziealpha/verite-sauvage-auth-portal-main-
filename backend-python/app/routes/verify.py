...
def verify_product(product_id: str):
    ...
    - When authentic, also ensures a VS Security Code is stored in codes.json
      along with key product metadata so the customer /customer-verify flow
      can display rich details even if the node is not running.
    """
    ...
    try:
        # Assumes your Solidity function is:
    ...
    # --- Authentic on-chain: ensure VS code + metadata are stored -------------
    # Build metadata for codes.json so customer-verify can show full details.
    meta = {
        "model": name,
        "color": color,
        "material": material,
    ...
    try:
        vs_code = register_code_for_product(pid, meta=meta)
    except Exception as e:
